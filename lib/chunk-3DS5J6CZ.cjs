...
var Password = class {
  ...
  async isPasswordPwned(page, password) {
    try {
      ...
      if (isResponseAvailable) {
        ...
        const messageContent = await page.$eval(this.pwnedMessage, (el) => _optionalChain([el, 'access', _ => _.textContent, 'optionalAccess', _2 => _2.trim, 'call', _3 => _3()]) || "");
        return {
  ...
  async isConfirmationMessageInDom(page) {
    try {
      return await _chunkZ7ZV6X7Ycjs.isElementInDom.call(void 0, page, this.messageContainer);
    } catch (error) {
      return false;
    }
  }
