...
var Pwned = class {
  ...
  async isPasswordPwned(page, password) {
    try {
      ...
      if (isResponseAvailable) {
        ...
        const messageContent = await page.$eval(
          this.pwnedMessage,
          (el) => _optionalChain([el, 'access', _4 => _4.textContent, 'optionalAccess', _5 => _5.trim, 'call', _6 => _6()]) || ""
        );
        return {
  ...
  async isConfirmationMessageInDom(page, selector) {
    try {
      return await _chunkT6OIEQM6cjs.isElementInDom.call(void 0, page, selector);
    } catch (error) {
      return false;
    }
  }
